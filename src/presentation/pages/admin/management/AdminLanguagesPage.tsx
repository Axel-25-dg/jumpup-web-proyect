  ...
  return (
    <div className="animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            ...
            <p className="text-base text-slate-500 dark:text-slate-400 max-w-lg font-medium">
              Administra los idiomas ofertados en JumpUp, configurando sus códigos MCER correspondientes.
            </p>
          </div>
          <Button
        ...
        <table className="w-full min-w-[800px] text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-900/10 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.02]">
              ...
              <th className="px-8 py-5 label-caps text-slate-400">Código MCER/ISO</th>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="rounded-none border-slate-900/10 p-8 sm:max-w-md">
          <DialogHeader className="mb-6">
            <DialogTitle className="text-2xl font-black uppercase tracking-tight">
              {editingLanguage ? 'Editar Idioma' : 'Nuevo Registro'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-6 py-4">
            <div className="space-y-2">
              <label className="label-caps text-slate-400 dark:text-slate-500">Nombre del Idioma</label>
              <input
                value={langName}
                onChange={(e) => setLangName(e.target.value)}
                placeholder="EJ. INGLÉS, FRANCÉS, ALEMÁN"
                className="w-full border border-slate-900/10 dark:border-white/10 bg-transparent py-3 px-4 text-[12px] font-bold uppercase tracking-widest outline-none focus:border-sky-500 transition-colors"
              />
            </div>
            <div className="space-y-2">
              <label className="label-caps text-slate-400 dark:text-slate-500">Código del Idioma</label>
              <input
                value={langCode}
                onChange={(e) => setLangCode(e.target.value.toUpperCase())}
                placeholder="EJ. EN, FR, DE"
                maxLength={5}
                className="w-full border border-slate-900/10 dark:border-white/10 bg-transparent py-3 px-4 text-[12px] font-bold uppercase tracking-widest outline-none focus:border-sky-500 transition-colors font-mono max-w-[150px]"
              />
              <p className="label-micro text-slate-400 mt-2 font-mono">CÓDIGO ISO ESTÁNDAR (MCER)</p>
            </div>
          </div>
          <DialogFooter className="sm:justify-end gap-3 mt-8">
            <Button
              ...
              className="rounded-none font-bold uppercase text-[10px] tracking-[0.2em] h-12 px-6"
            >
              Cancelar
            </Button>
            <Button
              ...
              disabled={isSaving}
              className="rounded-none bg-sky-500 hover:bg-sky-600 text-white font-bold uppercase text-[10px] tracking-[0.2em] h-12 px-8 transition-all"
            >
              {isSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
              {editingLanguage ? 'Actualizar Registro' : 'Guardar Idioma'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent className="rounded-none border-slate-900/10">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center text-sm font-medium py-4">
              ¿Está seguro de que desea eliminar el idioma <span className="text-slate-900 dark:text-white font-bold uppercase">"{langToDelete?.name} ({langToDelete?.code})"</span>? Esta acción podría afectar a los cursos vinculados.
            </AlertDialogDescription>
          </AlertDialogHeader>
